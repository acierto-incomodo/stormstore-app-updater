...
async function load(force = false) {
  ...
  try {
    const newApps = await window.api.getEpicGames();
    allApps = newApps;
    renderApps();
  } catch (e) {
    console.error(e);
    appsContainer.innerHTML = "<p>Error cargando juegos de Epic Games.</p>";
  } finally {
    if (force && refreshBtn) {
...
function renderApps() {
  ...
  if (filtered.length === 0) {
    appsContainer.innerHTML =
      "<p>No se encontraron juegos de Epic Games instalados.</p>";
    return;
  }
